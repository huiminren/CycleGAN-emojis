...
def training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, opts):
    ...
    for iteration in range(1, opts.train_iters+1):
        ...
        real_labels_X = torch.full((images_X.size(0),), real_label, device=device)
        real_labels_Y = torch.full((images_Y.size(0),), real_label, device=device)
        fake_labels_X = torch.full((images_X.size(0),), fake_label, device=device)
        fake_labels_Y = torch.full((images_Y.size(0),), fake_label, device=device)
            
        # 1. Compute the discriminator losses on real images
        if opts.loss_fn == 'div':
            D_X_loss = ((D_X(images_X)-1)**2).sum()/len(images_X) 
            D_Y_loss = ((D_Y(images_Y)-1)**2).sum()/len(images_Y) 
        elif opts.loss_fn =='torch_mean':
            D_X_loss =  torch.mean((D_X(images_X)-1)**2)
            D_Y_loss = torch.mean((D_Y(images_Y)-1)**2)
        elif opts.loss_fn =='criterion':
            D_X_loss = criterion(D_X(images_X),real_labels_X)
            D_Y_loss = criterion(D_Y(images_Y),real_labels_Y)
        ...
        if opts.loss_fn == 'div':
            ...
            D_X_loss = criterion(D_X(fake_X),fake_labels_Y)
        ...
        if opts.loss_fn == 'div':
            ...
            D_Y_loss = criterion(D_Y(fake_Y),fake_labels_X)
        ...
        if opts.loss_fn == 'div':
            ...
            g_loss = criterion(D_X(fake_X),real_labels_Y) 
        ...
        if opts.loss_fn == 'div':
            ...
            g_loss = criterion(D_Y(fake_Y),real_labels_X)

        if opts.use_cycle_consistency_loss:
            reconstructed_X = G_YtoX(fake_Y)
            # 3. Compute the cycle consistency loss (the reconstruction loss)
            if opts.loss_fn == 'div':
                cycle_consistency_loss = ((images_X-reconstructed_X)**2).sum()/len(images_X)
            elif opts.loss_fn == 'torch_mean':
        ...
        ttl_d_loss.append(d_real_loss.item()+D_Y_loss.item()+D_X_loss.item()+d_fake_loss.item())
        ttl_g_loss.append(g_loss.item())
...
#                     D_X_loss.item(), d_fake_loss.item(), g_loss.item()))
    ...
    showloss(ttl_g_loss, ttl_d_loss, opts.sample_dir)
...
def create_parser():
    ...
    parser.add_argument('--loss_fn',type=str, default='criterion', choices=['criterion', 'torch_mean', 'div'], help='Choose loss function')
